...
import { credentialRefForServer, setCredential, unsetCredential } from "./mcp/credentials.js";
...
async function buildInputFromArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        else if (arg === "--bearer" || arg === "--bearer-token") {
            i += 1;
            if (i >= args.length)
                throw new Error("--bearer 需要一个令牌参数");
            flags.bearerToken = args[i];
            flags.authType = "bearer";
        }
        else if (arg === "--timeout") {
            i += 1;
            if (i >= args.length)
    ...
    if (flags.stdio === true) {
        ...
        const authType = flags.authType === "bearer" ? "bearer" : "none";
        input = mcpServerInputSchema.parse({ ...common, transport: "streamable-http", url: flags.url, authType, bearerToken: flags.bearerToken, headers: parsePairs(flags.headers) });
    }
...
export async function runMcpCli(args) {
    ...
    if (command === "add") {
        ...
        if (built.input.transport === "streamable-http" && built.input.authType === "bearer" && built.input.bearerToken) {
            await setCredential(credentialRefForServer(built.input.serverName), built.input.bearerToken);
        }
    ...
    if (command === "remove" || command === "enable" || command === "disable") {
        ...
        if (command === "remove") {
            ...
            await unsetCredential(credentialRefForServer(name));
