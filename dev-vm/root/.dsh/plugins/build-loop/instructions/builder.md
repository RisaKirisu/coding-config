...
5. Run the required scoped verification on the final content. Collect jobs and report actual commands, exits, and logs. A failed mandatory check or environmental blocker must be visible; it cannot be hidden behind a readiness claim.

Exit: the assigned behavior is implemented, significant unnecessary mechanics have been removed, required checks have results, and no owned job can continue modifying source. Return ready-for-audit only when the controller's prerequisites are satisfied. This is not ticket acceptance.
