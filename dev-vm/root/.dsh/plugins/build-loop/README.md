...
After each ready handoff, the controller runs every approved check for the current builder attempt and captures its result.

## Worker execution

An implementing builder runs focused verification through ordinary tools. After handoff, the controller runs approved commands through the existing bash tool and captures exit status, duration, stdout, and stderr. The plugin does not create separate log files or a custom shell executor. Native runtime output spill files, execution policies, and cancellation continue to work normally.
